...
(function($){
	...
	$.widget("ui.editDateRangeSliderLabel", $.ui.rangeSliderLabel, {
		options: {
			type: "text",
			step: false,
			id: ""
		...
		_createInput: function(){
			this._input = $("<input type='" + this.options.type + "' />")
				.addClass("ui-editDateRangeSlider-inputValue")
				.appendTo(this._valueContainer);

            this._input.datepicker({ minDate: -1, maxDate: "+1Y", onSelect: function() {
                $(this).change();
            } });
			...
			this._input.bind('change', $.proxy(this._onChange, this));
                      /*
		...
		_onChange: function(){
			var value = this._returnCheckedValue(new Date(this._input.val()));
